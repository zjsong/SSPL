...
__all__ = ['ResNet', 'ResNet18', 'resnet34', 'resnet50', 'resnet101',
           ...
           'wide_resnet50_2', 'wide_resnet101_2', 'VGGish']
...
class ResNet(nn.Module):
    ...
    def _forward_impl(self, x):
        ...
        x = self.layer2(x)
        x = self.layer3(x)
        x = self.layer4(x)     # up to the final conv layer
        # print('size of sound feature map: ', x.size())   #  torch.Size([8, 512, 17, 18])
...
def _resnet(arch, block, layers, pretrained, progress, modal, resnet18_imgnet, **kwargs):
    model = ResNet(block, layers, modal, **kwargs)
    if pretrained:
        print('load pretrained res-18')
        model.load_state_dict(torch.load(resnet18_imgnet))

    return model


def ResNet18_(pretrained=False, progress=True, modal='vision', resnet18_imgnet=None, **kwargs):
