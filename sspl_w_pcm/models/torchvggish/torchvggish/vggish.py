...
class VGGish(VGG):
    def __init__(self, weights_vggish, weights_vggish_pca=None, out_dim=512,
        ...
        if pretrained:
            state_dict = torch.load(weights_vggish)
            super().load_state_dict(state_dict)

        self.preprocess = preprocess   # False, because it has been pre-processed during data loading
        self.postprocess = postprocess
        if self.postprocess:
            self.pproc = Postprocessor()
            if pretrained:
                state_dict = torch.load(weights_vggish_pca)
